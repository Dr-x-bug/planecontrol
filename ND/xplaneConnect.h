...
#include <ws2tcpip.h>
#if defined(_MSC_VER)
#pragma comment(lib,"ws2_32.lib") //Winsock Library (MSVC only)
#endif
...
/// Gets the attitude information for the specified aircraft.
///
/// \param sock   The socket used to send the command and receive the response.
/// \param values An array to store the attitude information returned by the
///               plugin. The format of values is [Pitch, Roll, Yaw]
/// \param ac     The aircraft to get the attitude of. 0 is the main/user's aircraft.
/// \returns      0 if successful, otherwise a negative value.
int getATT(XPCSocket sock, double values[3], char ac);
